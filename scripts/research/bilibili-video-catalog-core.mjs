...
export function assertVideoCatalog(catalog) {
  ...
  invariant(
    JSON.stringify(Object.keys(catalog.folders).sort()) ===
      JSON.stringify(['millionShots', 'siteFirsts']),
    'folders 只能包含百万直拍与全站第一目录',
  )
  invariant(!Object.hasOwn(catalog, 'streamPlaylist'), '视频目录不得包含旧刷播播放列表')
...
function publicVideoBvids(catalog) {
  return new Set([
...
export function buildPublicVideoCatalog(catalog) {
  ...
  const includedBvids = publicVideoBvids(catalog)
  return {
    schemaVersion: 1,
    ...
    videos: Object.fromEntries(
      catalog.videos
        .filter((video) => includedBvids.has(video.bvid))
        .map((video) => [video.bvid, favoriteVideoForBvid(catalog, video.bvid)]),
    ),
