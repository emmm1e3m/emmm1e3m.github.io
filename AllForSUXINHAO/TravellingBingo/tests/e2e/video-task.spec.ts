...
test('390px 移动端播放器在展开与收起后都避开房屋角落控件', async ({ page }, testInfo) => {
  ...
  await enterDialog.getByRole('button', { name: '进入现实维度' }).click()
  const trend = page.locator('[data-hotspot="二楼电脑·冲热"]')
  await expect(trend).toBeVisible()
  await trend.click()
  const unavailable = page.getByRole('dialog', { name: '冲热请使用电脑端' })
  await expect(unavailable).toContainText('冲热功能目前只在电脑端开放')
  await expect(unavailable).not.toContainText(/鼠标|触控板|指针/u)
