...
describe('V7 刷播会话历史', () => {
  ...
  it('严格 schema 拒绝额外字段、重复会话、非因果时间和会话轮次超出累计值', () => {
    const valid = createInitialGameState({ now: 1_000, seed: 'v7-schema' })
    expect(gameStateV7Schema.safeParse(valid).success).toBe(true)
