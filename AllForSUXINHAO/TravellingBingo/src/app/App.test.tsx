...
function readyActivityGame(kind: 'travel' | 'rest', companionDays = 4): GameState {
  ...
  return {
    ...
    wardrobe: {
      ...initial.wardrobe,
      shop: {
        companionDay: companionDays,
        assetIds: generateWardrobeShop(
          initial.random.seed,
          companionDays,
          initial.wardrobe.ownedAssetIds,
        ),
      },
    },
    activeActivity: {
...
describe('旅行饼狗应用控制器', () => {
  ...
  it('DEBUG 一键全收集仍通过 App 的领域动作写入同一份游戏状态', async () => {
    ...
    expect(screen.getByText('DEBUG：新增 31 项收藏、好友与衣服记录。')).toBeVisible()
    ...
    expect(screen.getByText('DEBUG：清理 31 项收藏、好友、衣服与造型记录。')).toBeVisible()
  })
