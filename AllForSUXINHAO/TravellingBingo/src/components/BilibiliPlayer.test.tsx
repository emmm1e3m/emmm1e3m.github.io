...
describe('BilibiliPlayer', () => {
  ...
  it('同一详情实例重渲染不重载，重新打开详情或切换来源都从头创建请求', () => {
    ...
    const hiddenFrame = iframe.closest('.persistent-bilibili-player__frame')
    expect(hiddenFrame).toHaveAttribute('inert')
    expect(hiddenFrame).toHaveAttribute('aria-hidden', 'true')
    expect(iframe).not.toHaveAttribute('inert')
