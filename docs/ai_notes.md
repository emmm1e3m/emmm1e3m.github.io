...
- `wardrobe.shop.companionDay` 必须等于 `profile.companionDays`；每个游戏日从日初未拥有的可售资产池确定性生成 `min(3, remaining)` 个互异商品。未拥有池不少于 3 件时为 3 件，不足时全取，全收集时为空。持久 `shop.assetIds` 长度为 0–3，严格导入按本日已购买商品仍可追溯到日初抽取结果来校验。衣架信息菜单只渲染当日商品中尚未拥有的条目，使用真实透明缩略图，并复用冰箱的购买卡片、金额、余额不足和禁用态；购买成功后条目立即消失，但当日不补入新商品。可买列表为空时不渲染“今天仍可购买”标题。系统日期变化、失败动作和取消操作不能刷新商品。
- 当前目录包含 24 个稳定资产 ID，其中 8 套服装、16 件配饰；`cream-apple-cape` 是新档与 V10→V11 迁移的初始赠送，其余价格平均约 5🍎。购买按目录价格原子扣除，不得重复购买。衣架信息菜单分别打开全屏 `MiracleWardrobePage` 与独立舞台测试；舞台不属于奇迹页。Miracle 使用 `100dvh` 紧凑壳层：头部、三页签和状态提示固定占位，余高给当前面板；桌面搭配按 library/stage/tools、合拍按 setup/stage/tools 各分三栏，pane 自身不滚，画布、图层区和变换按钮常驻，只把 saved looks、人物造型、素材列表与明信片 wall 设为内部滚动容器。宽度不超过 960px 时撤销子列表滚动，按 DOM 顺序改为单列并由活动 panel 统一纵向滚动，触控面不小于 44px。
- 可搭配目标为 `bingo | FriendId`，好友必须已经出现在 `state.friends`。同一角色最多保存 8 套具名造型；名称 1–20 字符，使用稳定 `lookId`，支持新建、更新与删除。角色海星体和所有衣服实例位于同一个 `.miracle-look-composition` 坐标系，该组合层固定为画布的 `0.5`；元素归一化位置与两轴尺寸都以组合层宽度为基准，`scaleX === scaleY` 时保持素材天然宽高比。编辑草稿留在组件内；保存时只写 `assetId`、稳定 `placementId` 与归一化 `x/y/scaleX/scaleY/rotation/z`，不写图片 URL、Blob、Data URL 或像素结果。搭配室用当前草稿渲染角色透明 PNG，下载只生成临时位图，不写回造型或存档。
- 每套形象最多 12 个元素；同一已购资产可以通过不同 `placementId` 重复放置。搭配与合拍的右栏都把图层列表放在素材选择器之前，素材按 `outfit | head | face | accessory | prop` 的非空分类筛选。移动使用主体拖拽，左上手柄控制非等比横纵拉伸，右下手柄控制等比缩放与旋转；🔼、🔽、🔄️、❌ 四个操作在同一行，分别前移、后移、恢复默认变换和移除，并以 `aria-label` / `title` 暴露完整动作名称。照片图层名称不再附加类型括注，同名项按出现顺序编号；界面移除三段重复的选择、拖动与素材提示。合拍角色数组必须包含 1–6 位可用角色，已认识好友可单独组成合拍。人物整体也持久化双轴变换。相册最多 40 张，照片冻结当时的轻量搭配快照，之后换装不能改变旧照片。
- 合拍复用现实苹果钟的 `PostcardPicker` 弹窗，只接收已拥有明信片选项，`includePlain=false`，也不传入已保存合拍，因此白纸与历史合拍都不能成为持久合拍背景。预览与 PNG 画布使用当前背景自身长宽比，完整铺满且不加遮罩；`clearPhotoCanvas()` 只清空参与者、独立装饰和当前选中项，不修改明信片或本地图片背景。
- 本地图片背景只存在 `MiracleWardrobePage` 的 `LocalPhotoBackground` 临时状态，支持 PNG、JPEG、WebP 与 AVIF；上传与构图入口不受已收藏明信片数量限制。预览 object URL 在上传失败、成功替换、显式改回明信片或组件卸载时释放；下载另建输入 object URL，并在 Canvas 完成后的 `finally` 释放。`draftPhoto.postcardId = null`，本地模式只调用带 `backgroundOverride` 的 `downloadWardrobePhoto()`，隐藏保存相册按钮，`savePhoto()` 仍有硬拒绝；不 dispatch `wardrobe/photo-create`，也不把文件、URL 或像素写入 `GameState`。
- 每张照片最多保存 12 个独立 `decorations`；它们直接属于照片、不属于人物，与人物共用照片级全局唯一 `z` 并可交错渲染，人物内部服饰继续使用各自局部 `z`。衣服收藏使用自适应方形缩略框与 `object-fit: contain`，完整居中展示素材。合拍相册列表固定以 4:3 `cover` 预览铺满卡片；详情使用 `mode="natural"` 按明信片原始比例计算媒体区，并以 `contain` 完整留在视口内。导出默认长边 2400，短边随背景比例计算，已保存合拍优先加载 960 明信片并回落 480 图。合拍独立于 `collections`，不得影响收藏进度。
...
- 灰色活动入口使用 `aria-disabled` 表达不愿意，但保留点击反馈；有活力魔法时点击单活动入口或共享热点都会直接 dispatch 使用动作，没有活力魔法时只显示拒绝，不能进入普通启动确认。
- ⚡瓶装速度魔法价格 2🍎。只允许在当前活动仍为 running 时使用；点击入口后直接消耗一瓶并把该活动的 `endsAt` 设为使用时间，不再打开确认对话框，已经 ready 时不消耗。
- ✨瓶装活力魔法价格 7🍎。点击单活动入口或共享热点时直接使用，不再打开确认对话框；使用后立即把三组意愿设为真、清除疲劳。效果持续七个由“饼屋活动成功领取”或“苹果钟完成专注与休息整轮”推进的相伴日，不是七个自然日。苹果钟推进到第七日时同样清除效果并按偏好序列恢复当日意愿；已有有效效果或所有意愿本就可用时拒绝并不消耗。
...
- 顶栏把导航入口放在左组、运行状态和资源入口放在右组，自适应收缩且不相互覆盖。现实分钟、活力、🍎、收藏墙和 DEBUG 均为可点击按钮；现实分钟执行返回游戏维度，活力执行点击饼狗。旧“状态正常”不再派生或显示；主游戏 HUD 只在独立刷播窗口处于 `opening | waiting | stopping` 时映射显示启动中、运行中或停止中，点击尝试聚焦该窗口并在窗口已关闭时返回刷播面板。轮次、倒计时和任务记录仍只在独立在线刷播工具展示。
...
- 苹果钟预设为 25+5、50+10、90+15 分钟。全屏层锁定启动时的 `PomodoroBackgroundRef`；明信片直接展示，奇迹合拍按持久构图动态重建，二者都以 `cover` 铺满背景并允许裁切，与可操作的游戏播放器控件、待办和饼狗共同组成焦点 UI。专注场景的 `MascotSprite` 固定使用 `pose="stream"`，并在 `.pomodoro-focus__mascot` 内专属裁去顶部 2%；房间中的同姿态不裁切。这只是视觉语义，不改变苹果钟领域状态。
