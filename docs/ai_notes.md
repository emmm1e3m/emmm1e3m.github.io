...
- 当前产品显示版本为 `v0.10`；业务状态为 `GameStateV11`、`schemaVersion: 11`，新导出封套中的 `gameVersion` 为 `0.10.0`。
...
- `src/domain/game/types.ts`：`GameStateV1`–`GameStateV11`、动作与 effect。
- `src/domain/game/migrateGameStateV1.ts`–`migrateGameStateV10.ts`：逐代迁移；冻结的 V1–V10 schema 不回写新字段。
- `src/app/gameStateSchema.ts`：严格历史 schema 联合与 V11 导出 schema。
...
wardrobe.shop / ownedAssetIds / looks / photos
```

历史 V6→V7 的刷播轮次转换、V7→V8 的旧设置初始化只服务冻结旧档解析；V11 仍保留这份兼容数据，但运行时不再接收或写入新的轮次/会话。V8→V9 增加现实停留租约，V9→V10 保留自测 BVID 并选择默认收藏夹。V10 已冻结为迁移历史；V10→V11 根据持久种子与 `companionDays` 初始化衣柜、从日初未拥有池生成的最多三件商品和空合拍相册，并把尚未领取的旧睡觉 `rewardPlan.baseApples` 归零；除此之外不重算旧活动、收藏、好友、旧刷播统计或现实时长。

当前面板、弹窗焦点、琴键按下状态、奖励弹窗和布局动画仍是 UI 瞬时状态，不写入存档。

## 奇迹饼狗

- `wardrobe.shop.companionDay` 必须等于 `profile.companionDays`；每个游戏日从日初未拥有的可售资产池确定性生成 `min(3, remaining)` 个互异商品。未拥有池不少于 3 件时为 3 件，不足时全取，全收集时为空。持久 `shop.assetIds` 长度为 0–3，严格导入按本日已购买商品仍可追溯到日初抽取结果来校验。衣架信息菜单只渲染当日商品中尚未拥有的条目；购买成功后条目立即消失，但当日 `shop.assetIds` 不补入新商品。系统日期变化、失败动作和取消操作不能刷新商品。
- 当前目录包含 24 个稳定资产 ID，其中 8 套服装、16 件配饰；`cream-apple-cape` 是新档与 V10→V11 迁移的初始赠送，其余价格平均约 5🍎。购买按目录价格原子扣除，不得重复购买。衣架信息菜单分别打开全屏 `MiracleWardrobePage` 与独立舞台测试；舞台不属于奇迹页。
- 可搭配目标为 `bingo | FriendId`，好友必须已经出现在 `state.friends`。同一角色最多保存 8 套具名造型；名称 1–20 字符，使用稳定 `lookId`，支持新建、更新与删除。编辑草稿留在组件内；保存时只写 `assetId`、稳定 `placementId` 与归一化 `x/y`、`scale`、`rotation`、唯一 `z`，不写图片 URL、Blob、Data URL 或像素结果。
- 每套形象最多 12 个元素；同一已购资产可以通过不同 `placementId` 重复放置。移动使用主体拖拽，缩放与旋转共用选中元素角落手柄；合拍角色数组必须包含 1–6 位，`bingo` 非必选，可以只包含已认识好友。相册最多 40 张。照片冻结当时的轻量搭配快照，之后换装不能改变旧照片。
- 合拍只选已拥有的明信片，预览与 PNG 画布使用明信片自身长宽比，背景完整铺满且不加遮罩；人物仍保存归一化变换。导出默认长边 2400，短边随明信片比例计算，优先加载 960 图并回落 480 图。合拍独立于 `collections`，不得影响收藏进度。
...
- DEBUG 的“一键全收集”同时获得三类收藏、全部好友与 24 件衣服，并把 `shop.assetIds` 清空；“清空收集”清空收藏、好友与全部 saved looks，`ownedAssetIds` 恢复为仅 `cream-apple-cape`，并在同一 `companionDay` 强制重建 3 件商品。历史合拍的轻量造型快照保留，只把已经失效的 `postcardId` 置空。不维护预制全收集存档。
...
- 顶栏把导航入口放在左组、运行状态和资源入口放在右组，自适应收缩且不相互覆盖。现实分钟、活力、🍎、收藏墙和 DEBUG 均为可点击按钮；现实分钟执行返回游戏维度，活力执行点击饼狗。旧“状态正常”不再派生或显示；刷播运行态只在 `StreamPanel` 与独立页展示。
...
- `StreamPanel` 把主说明、设备检查、弹窗权限、时长限制和继续运行提醒合并为一个分点 guidance 容器，不再拆成多块非标准提示。`src/stream-player/` 是“在线刷播工具”独立页实现。页面可从 URL 自动带入设置，也可在自身 UI 修改收藏夹、自测视频和定时停止后手动开始。固定使用同源 `favourites/<favoriteId>.txt`，`fetch(..., { credentials: 'same-origin' })` 后逐行校验 BV；运行时没有 B 站收藏夹 API、HTML 代理或 CORS 读取。
...
- 会话可快照 0–24 小时的定时停止，0/null 为不限时。轮次、下一轮倒计时、当前状态及最近 10 次任务只在独立页展示并写入独立页自己的 `localStorage`；主游戏不接收、不渲染也不写入新的轮次或会话统计，`reality.streamHistory` 仅作为冻结兼容字段随旧存档保留。运行中的 iframe、定时器、保活音频和独立页 DEBUG 状态不进入 `.bingo`。
...
- 标题页在两个圆形头像后、缓存/导入记录前渲染 `UpdateNoticeCard`，房间左上角 `#️⃣` 复用 `UpdateNoticeDialog`；两处读取同一 `CURRENT_UPDATE_NOTICE.version = 'v0.10'`，并同时显示“刷播现在可以正常使用了”以及奇迹饼狗、多套造型和合拍相册条目。
...
- `prepareBrowserCache` 只要 payload 迁移/规范/协调/暂停旧页面租约后形成新对象或 `gameVersion` 过旧，就用 `updateBrowserGameCache` 同步 V11 payload、`0.10.0` 与 `updatedAt`；保留 `saveId`、`firstCachedAt`、`lastPeriodicBackupRequestedAt` 和外层 `cacheVersion: 1`。
...
`npm run verify` 不包含 Playwright；浏览器 E2E 必须单独执行。测试播放器时只能断言请求 URL、iframe 生命周期、任务事件和持久状态，不能断言跨域 iframe 的真实播放状态。现实刷播测试应覆盖 BV/完整链接规范化与 V11 持久化、两份 TXT 唯一来源、`SUperView` 窗口与“在线刷播工具”标题、独立页参数/手动设置/自动开始、每轮随机且自测末尾、固定 5 秒创建间隔、最后创建后墙钟等待 310 秒、轮末统一销毁、0/空不限时、独立 DEBUG 持久/关闭、默认隐藏播放器、页面常驻保活、独立页统计、主游戏不写入新统计、冻结兼容字段往返、停止清理，以及无代理/运行时收藏夹 API 请求；不得把 iframe `load`、本地轮次或窗口成功创建解释为 B 站真实播放或计数。迁移测试需固定 V10→V11 将尚未领取的旧睡觉 `baseApples` 归零、其他活动快照不变；奇迹饼狗另需覆盖衣架未拥有列表、日初 `min(3, remaining)` 商店、购买后不补位、平均价格、购买原子性、多套造型、已遇好友权限、角落手柄变换往返、1–6 位合拍与饼狗非必选、动态明信片比例、相册删除、PNG 导出和 `.bingo` 不含位图。
