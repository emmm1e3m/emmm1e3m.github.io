...
- 当前业务状态为 `GameStateV10`，`schemaVersion: 10`；新导出封套中的 `gameVersion` 为 `0.10.0-demo.1`。
...
- `src/domain/game/types.ts`：`GameStateV1`–`GameStateV10`、动作与 effect。
- `src/domain/game/migrateGameStateV1.ts`–`migrateGameStateV9.ts`：逐代迁移；冻结的 V1–V9 schema 不回写新字段。
- `src/app/gameStateSchema.ts`：严格历史 schema 联合与 V10 导出 schema。
...
V1–V5 先迁到 V6 并初始化空刷播历史，再迁到 V7。历史 V6→V7 迁移会把每条 `recentRounds` 独立转为一条 `legacy-round-*` 会话，不按时间阈值猜测合并；V7→V8 初始化 `selfTestBvid` 与已经冻结的 `dimensionPenetrationEnabled`，V8→V9 为现实停留增加可暂停租约。旧 V8 活跃停留因没有心跳证据而从 0 开始累计，不使用缓存更新时间推断在线时长。V9→V10 保留 `selfTestBvid`，删除当前态的维度穿透并写入 `favoriteId: 3682220021`。当前每轮 progress 只增加累计轮次；session end 才追加最近任务，重复的完全相同 end 幂等，冲突内容拒绝。
...
- `features/reality/stream/parser.ts` 接受单个可选裸 BVID 或 `http(s)` B 站完整视频链接，规范为 BVID 后写入 `reality.streamSettings.selfTestBvid`；`favoriteId` 只能取 `3682220021 | 3986840044`。V10 当前刷播设置只持久化这两个字段。
- `features/reality/stream/useStreamPlayback.ts` 只在用户手势内同步 `window.open()` 一个命名的 430 × 760 独立窗口，并通过查询参数传入 `favoriteId`、`selfTest`、`stopHours`、`sessionId` 与 `autostart=1`。主游戏不创建 B 站 iframe，也不负责视频顺序；弹窗被拦截时只显示授权后重新开始。
- `src/stream-player/` 是独立页实现。页面可从 URL 自动带入设置，也可在自身 UI 修改收藏夹、自测视频和定时停止后手动开始。固定使用同源 `favourites/<favoriteId>.txt`，`fetch(..., { credentials: 'same-origin' })` 后逐行校验 BV；运行时没有 B 站收藏夹 API、HTML 代理或 CORS 读取。
- `StreamRoundScheduler` 每轮用 Fisher–Yates 重新打乱去重后的收藏夹，自测 BVID 去重后固定追加在末尾；首个 iframe 立即创建，之后固定每 5 秒创建一个。URL 固定带 `autoplay=1&muted=1&danmaku=0&t=0`。
...
- 独立页启动时创建循环的低音量 WAV 保活音频，结束或卸载时释放。播放器 iframe 始终真实存在；默认 CSS 隐藏，只有独立页 DEBUG 中选择“显示实际播放器”才展示。
- 隐藏 DEBUG 由独立页大标题连续点击 5 次后出现密码框，密码严格为 `SUperView`。解锁后可以修改独立页轮次间隔并切换播放器显示；主游戏 DEBUG 已删除刷播轮次字段，两个 DEBUG 完全解耦。
- 会话可快照 0–24 小时的定时停止，0/null 为不限时。每轮完成通过同源 `postMessage` 派发 `reality/stream-session-progress`，会话结束再派发一次 `reality/stream-session-end`；独立页同时把最近 10 次完成/停止任务写入同源 `localStorage`，主游戏聚合两处记录并按 `sessionId` 去重。运行中的 iframe、定时器、保活音频和 DEBUG 设置不持久化。
...
- 标题页在两个圆形头像后、缓存/导入记录前渲染 `UpdateNoticeCard`，房间左上角 `#️⃣` 复用 `UpdateNoticeDialog`；两处读取同一 `CURRENT_UPDATE_NOTICE`。当前公告必须包含精确提示“目前刷播功能不稳定，请暂时不要通过此方式刷播”。
...
- `prepareBrowserCache` 只要 payload 迁移/规范/协调/暂停旧页面租约后形成新对象或 `gameVersion` 过旧，就用 `updateBrowserGameCache` 同步 V10 payload、`0.10.0-demo.1` 与 `updatedAt`；保留 `saveId`、`firstCachedAt`、`lastPeriodicBackupRequestedAt` 和外层 `cacheVersion: 1`。
...
`npm run verify` 不包含 Playwright；浏览器 E2E 必须单独执行。测试播放器时只能断言请求 URL、iframe 生命周期、任务事件和持久状态，不能断言跨域 iframe 的真实播放状态。现实刷播测试应覆盖 BV/完整链接规范化与 V10 持久化、两份 TXT 唯一来源、独立页参数/手动设置/自动开始、每轮随机且自测末尾、固定 5 秒创建间隔、最后创建后墙钟等待 310 秒、轮末统一销毁、0/空不限时、独立 DEBUG 五击与密码、默认隐藏播放器、保活音频、同源消息和最近 10 次任务、停止清理，以及无代理/运行时收藏夹 API 请求；不得把 iframe `load`、本地轮次或窗口成功创建解释为 B 站真实播放或计数。
